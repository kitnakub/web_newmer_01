...
const Inverse: React.FC = () => {
  ...
  const calculateInverse = () => {
    try {
      setErrorMsg("");
      const A_inv = inv(matrixA);
      const result = multiply(A_inv, vectorB);
      const n = matrixA.length;

      const outputSolutions: Solution[] = [];
      for (let i = 0; i < n; i++) {
        const val = Number.isInteger(result[i])
          ? result[i]
          : format(fraction(result[i]), { fraction: "ratio" });
        outputSolutions.push({ variable: `X${i + 1}`, value: val });
      }

      setSolutions(outputSolutions);
      setShowOutput(true);
    } catch (error) {
      setErrorMsg("Matrix ไม่สามารถหา Inverse ได้ (Determinant = 0)");
    }
  };
